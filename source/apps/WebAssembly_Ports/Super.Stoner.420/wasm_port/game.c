...
void reload_level() {
    ...
    if(custom_level == 0)
        sprintf(sbuf,"/assets/%slevel/level%d.sml", level_str[cur_levels], ++cur_level);
    else
...
static void init() {
    ...
    font = SDL_InitFont(get_path("/assets", "font/system.mxf"));
    cfont = SDL_InitFont(get_path("/assets", "font/e.mxf"));
    //SDL_FillRect(front, 0, 0);
    ...
    particles[0] = SDL_LoadBMP(get_path("/assets", "img/shot.bmp"));
    lsd = SDL_LoadBMP(get_path("/assets", "img/lsd.bmp"));
    logo = SDL_LoadBMP(get_path("/assets", "img/logo.bmp"));
    for(i = 0; img_str[i] != 0; i++) {
        static char sbuf[256];
        sprintf(sbuf, "img/%s", img_str[i]);
        gfx[i] = SDL_LoadBMP(get_path("/assets", sbuf));
        if(!gfx[i])
    ...
    for(i = 0; hstr[i] != 0; i++) {
        ...
        hgfx[i] = SDL_LoadBMP(get_path("/assets", sbuf));
        if(!hgfx[i])
            fprintf(stderr, "Error couldnt load graphic %s\n", sbuf);
    }
    ...
    for(i = 0; i < COLLECT_NUM; i++) {
        ...
        collect[i] = SDL_LoadBMP(get_path("/assets", sbuf));
        if(!collect[i])
            fprintf(stderr, "Error couldnt load graphic %s\n", sbuf);
    }
    
    {
        ...
        for( i = 0; ev[i] != 0; i++ ) {
            ...
            for( z = 0; z < 10; z++ ) {
                ...
                evil_gfx[i].gfx[z] = SDL_LoadBMP(get_path("/assets", sbuf));
                if(!evil_gfx[i].gfx[z])
    ...
    //SDL_Flip(front);
    bg = SDL_LoadBMP(get_path("/assets", "img/bg.bmp"));
